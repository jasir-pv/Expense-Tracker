...
export function UpcomingExpenseItem({
  ...
  return (
    <div
      className={`flex items-center gap-3 py-3 px-3 border-b dark:border-gray-700 last:border-0 rounded-lg transition-colors ${
        isPaid
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className={`font-medium text-sm truncate ${isPaid || isSkipped ? 'line-through' : ''} text-gray-900 dark:text-white`}>
            {expense.title}
          </p>
          {isOverdue && (
        ...
        <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
          <span>{formatDate(new Date(expense.dueDate))}</span>
          <span>•</span>
          <span>{getFrequencyLabel(expense.frequency, expense.interval)}</span>
          {expense.autoConvert && (
            <>
              <span>•</span>
              <span className="text-blue-600 dark:text-blue-400">Auto-convert</span>
            </>
          )}
        </div>
      </div>
      <div className="text-right flex-shrink-0 mr-2">
        <p className={`font-semibold ${isPaid || isSkipped ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white'}`}>
          {formatCurrency(expense.amount)}
        </p>
        <p className="text-xs text-gray-600 dark:text-gray-400">{expense.category.name}</p>
      </div>
